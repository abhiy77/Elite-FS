...
Your task is to find out, the length of the longest subsequence that is a
toggle sequence.

Toggle Sequence means, the difference between the consecutive numbers
are alternate positive and negative.
The sequence should be either +, -, +, -, +, -,..... numbers
or -, +, -, +, -, +,..... numbers.
...
Note: A sequence with fewer than two elements is not a toggle sequence.
...
public class ToggleSequence {
	...
	public static int toggleSequence(int[] temp) {
		int n = temp.length;
		int dp[][] = new int[n][2];
		for(int a[] : dp) {
			Arrays.fill(a, 1);
	...
	public static void main(String[] args) {
		...
		System.out.println(toggleSequence(temp) + 1);
	}
}