...
The building has K rooms inside indexed 0,1,..-K-1, 
and the height of the rooms are different.
All the rooms connected with each other and has only one entry point, 
...
To keep the items into the rooms, delivery agent has to follow these rules:
 ...
 - As there is only one entry point, If the height of some room is less than the height of an item,
...
public class CourierService {
	...
	public static void main(String[] args) {
		...
		sc.nextLine();
		int item_heights[] = Arrays.asList(sc.nextLine().split(" ")).stream().mapToInt(Integer::parseInt).toArray();
		int room_heights[] = Arrays.asList(sc.nextLine().split(" ")).stream().mapToInt(Integer::parseInt).toArray();
		int min_req[] = new int[rooms];
		min_req[0] = room_heights[0];
		for(int i=1;i<rooms;i++) {
			min_req[i] = Math.min(min_req[i-1], room_heights[i]);
		}
		Arrays.sort(item_heights);
		
		int count = 0;
		int right_limit = min_req.length-1;
		
		for(int i=0;i<items;i++) {
			for(int j=right_limit;j>=0;j--) {
				if(min_req[j] >= item_heights[i]) {
					count++;
					right_limit = j-1;
					break;
				}
